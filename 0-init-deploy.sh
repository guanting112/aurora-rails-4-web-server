...
load_setting_file()
{
  . /tmp/aurora-rails-4-system-environment/0-machine-setting.sh
  rm /tmp/aurora-rails-4-system-environment/0-machine-setting.sh  
}

copy_setup_scripts()
{
  sudo cp -R /tmp/aurora-rails-4-system-environment/ /home/$DEPLOY_USER_NAME/aurora-rails-4-system-environment
  sudo rm -rfv /tmp/aurora-rails-4-system-environment/ > /dev/null
  sudo rm -rfv /home/$DEPLOY_USER_NAME/aurora-rails-4-system-environment/0-machine-setting.sh > /dev/null
  sudo rm -rfv /home/$DEPLOY_USER_NAME/aurora-rails-4-system-environment/0-init-deploy.sh > /dev/null
  sudo chown $DEPLOY_USER_NAME:$DEPLOY_USER_NAME /home/$DEPLOY_USER_NAME/aurora-rails-4-system-environment/*
  sudo chmod +x /home/$DEPLOY_USER_NAME/aurora-rails-4-system-environment/1-setup.sh
}
